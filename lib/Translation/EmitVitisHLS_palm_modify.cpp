...
static SmallString<16> getTypeName(Type valType) {
  ...
  // else if (auto fixedType = valType.dyn_cast<codo::FixedType>())
  //   return SmallString<16>(
  //       "ap_fixed<" + std::to_string(fixedType.getWidth()) + ", " +
  //       std::to_string(fixedType.getWidth() - fixedType.getFrac()) + ">");

  // else if (auto ufixedType = valType.dyn_cast<codo::UFixedType>())
  //   return SmallString<16>(
...
public:
  ...
  void emitConstBuffer(codo::ConstBufferOp op);
  void emitStreamChannel(codo::StreamOp op);
  void emitStreamRead(codo::StreamReadOp op);
  void emitStreamWrite(codo::StreamWriteOp op);
  ...
  void emitAffineFor(mlir::affine::AffineForOp op);
  void emitAffineIf(mlir::affine::AffineIfOp op);
  void emitAffineParallel(mlir::affine::AffineParallelOp op);
  void emitAffineApply(mlir::affine::AffineApplyOp op);
  template <typename OpType>
  void emitAffineMaxMin(OpType op, const char *syntax);
  void emitAffineLoad(mlir::affine::AffineLoadOp op);
  void emitAffineStore(mlir::affine::AffineStoreOp op);
  void emitAffineYield(mlir::affine::AffineYieldOp op);
  ...
  // void emitGetGlobalFixed(codo::GetGlobalFixedOp op);
  ...
  // void emitGetBit(codo::GetIntBitOp op);
  // void emitSetBit(codo::SetIntBitOp op);
  // void emitGetSlice(codo::GetIntSliceOp op);
  // void emitSetSlice(codo::SetIntSliceOp op);
  // void emitBitReverse(codo::BitReverseOp op);
...
public:
  ...
  bool visitOp(codo::ConstBufferOp op) { return emitter.emitConstBuffer(op), true; }
  bool visitOp(codo::StreamOp op) { return emitter.emitStreamChannel(op), true; }
  bool visitOp(codo::StreamReadOp op) { return emitter.emitStreamRead(op), true; }
  bool visitOp(codo::StreamWriteOp op) { return emitter.emitStreamWrite(op), true; }
  /// SCF statements.
  ...
  bool visitOp(affine::AffineMaxOp op) {
    return emitter.emitAffineMaxMin<affine::AffineMaxOp>(op, "max"), true;
  }
  bool visitOp(affine::AffineMinOp op) {
    return emitter.emitAffineMaxMin<affine::AffineMinOp>(op, "min"), true;
  }
  ...
  // bool visitOp(codo::GetGlobalFixedOp op) {
...
public:
  ...
  // bool visitOp(codo::GetIntBitOp op) { return emitter.emitGetBit(op), true; }
  // bool visitOp(codo::SetIntBitOp op) { return emitter.emitSetBit(op), true; }
  // bool visitOp(codo::GetIntSliceOp op) { return emitter.emitGetSlice(op), true; }
  // bool visitOp(codo::SetIntSliceOp op) { return emitter.emitSetSlice(op), true; }
  // bool visitOp(codo::BitReverseOp op) {
  ...
  // bool visitOp(codo::FixedToFloatOp op) {
  //   return emitter.emitCast<codo::FixedToFloatOp>(op), true;
  // }
  // bool visitOp(codo::FloatToFixedOp op) {
  //   return emitter.emitCast<codo::FloatToFixedOp>(op), true;
  // }
  // bool visitOp(codo::IntToFixedOp op) {
  //   return emitter.emitCast<codo::IntToFixedOp>(op), true;
  // }
  // bool visitOp(codo::FixedToIntOp op) {
  //   return emitter.emitCast<codo::FixedToIntOp>(op), true;
  // }
  // bool visitOp(codo::FixedToFixedOp op) {
  //   return emitter.emitCast<codo::FixedToFixedOp>(op), true;
  // }
  ...
  // /// HCL operations.
  // bool visitOp(codo::CreateLoopHandleOp op) { return true; }
  // bool visitOp(codo::CreateOpHandleOp op) { return true; }

  // /// Fixed points
  // bool visitOp(codo::AddFixedOp op) { return emitter.emitBinary(op, "+"), true; }
  // bool visitOp(codo::SubFixedOp op) { return emitter.emitBinary(op, "-"), true; }
  // bool visitOp(codo::MulFixedOp op) { return emitter.emitBinary(op, "*"), true; }
  // bool visitOp(codo::DivFixedOp op) { return emitter.emitBinary(op, "/"), true; }
  // bool visitOp(codo::CmpFixedOp op);
  // bool visitOp(codo::MinFixedOp op) {
  //   return emitter.emitMaxMin(op, "min"), true;
  // }
  // bool visitOp(codo::MaxFixedOp op) {
  //   return emitter.emitMaxMin(op, "max"), true;
  // }
...
// bool ExprVisitor::visitOp(codo::CmpFixedOp op) {
//   switch (op.getPredicate()) {
//   case codo::CmpFixedPredicate::eq:
//     return emitter.emitBinary(op, "=="), true;
//   case codo::CmpFixedPredicate::ne:
//     return emitter.emitBinary(op, "!="), true;
//   case codo::CmpFixedPredicate::slt:
//   case codo::CmpFixedPredicate::ult:
//     return emitter.emitBinary(op, "<"), true;
//   case codo::CmpFixedPredicate::sle:
//   case codo::CmpFixedPredicate::ule:
//     return emitter.emitBinary(op, "<="), true;
//   case codo::CmpFixedPredicate::sgt:
//   case codo::CmpFixedPredicate::ugt:
//     return emitter.emitBinary(op, ">"), true;
//   case codo::CmpFixedPredicate::sge:
//   case codo::CmpFixedPredicate::uge:
//     return emitter.emitBinary(op, ">="), true;
//   default:
//     op.emitError("has unsupported compare type.");
//     return false;
...
/// HLS operation emitters.
void ModuleEmitter::emitConstBuffer(codo::ConstBufferOp op) {
  emitConstant(op);
  emitArrayDirectives(op.getResult());
}

void ModuleEmitter::emitStreamChannel(codo::StreamOp op) {
  indent();
...
void ModuleEmitter::emitStreamRead(codo::StreamReadOp op) {
  indent();
  if (op.getResult()) {
...
void ModuleEmitter::emitStreamWrite(codo::StreamWriteOp op) {
  indent();
  emitValue(op.getChannel());
  os << ".write(";
...
// void ModuleEmitter::emitGetGlobalFixed(codo::GetGlobalFixedOp op) {
//   indent();
//   os << "// const ";
//   Value result = op.getResult();
//   fixUnsignedType(result, op->hasAttr("unsigned"));
...
void ModuleEmitter::emitMemCpy(memref::CopyOp op) {
  ...
  os << type.getNumElements() << " * sizeof("
     << getTypeName(op.getTarget().getType()) << "));";
  emitInfoAndNewLine(op);
  os << "\n";
}
...
// void ModuleEmitter::emitGetBit(codo::GetIntBitOp op) {
//   indent();
//   Value result = op.getResult();
//   fixUnsignedType(result, op->hasAttr("unsigned"));
...
// void ModuleEmitter::emitSetBit(codo::SetIntBitOp op) {
//   indent();
//   // generate ap_int types
//   os << "ap_int<" << op.getNum().getType().getIntOrFloatBitWidth() << "> ";
//   emitValue(op.getNum());
...
// void ModuleEmitter::emitGetSlice(codo::GetIntSliceOp op) {
//   indent();
//   Value result = op.getResult();
//   fixUnsignedType(result, op->hasAttr("unsigned"));
//   // generate ap_int types
...
// void ModuleEmitter::emitSetSlice(codo::SetIntSliceOp op) {
//   indent();
//   // T v;
//   // v(a, b) = x;
...
// void ModuleEmitter::emitBitReverse(codo::BitReverseOp op) {
//   indent();
//   Value result = op.getResult();
//   fixUnsignedType(result, op->hasAttr("unsigned"));
//   emitValue(result);
...
void ModuleEmitter::emitFunctionDirectives(func::FuncOp func,
  ...
  llvm::outs() << "emitFunctionDirectives";
  llvm::outs() << '\n';
  // Emit other pragmas for function ports.
  for (auto &port : portList){
    llvm::outs() << "emitportList";
    llvm::outs() << '\n';
...
void ModuleEmitter::emitFunction(func::FuncOp func) {
  ...
  for (auto &arg : func.getArguments()) {
    ...
    llvm::outs() << "emitgetArguments";
    llvm::outs() << '\n';
    type.dump();

    if (type.isa<MemRefType>()){
      llvm::outs() << "emitArrayDecl";
      llvm::outs() << '\n';
      emitArrayDecl(arg);
    }
      
    else if (type.isa<codo::StreamType>())
      emitValue(arg, /*rank=*/0, /*isPtr=*/false, /*name=*/"temp");
    else
      emitValue(arg);
...
#include <stdint.h>
  ...
  if (module.getName().has_value() && module.getName().value() == "host") {
    ...
    //   //   emitGlobal(cst);
    //   // TODO
    //   else if (!isa<mlir::ml_program::GlobalOp>(op))
