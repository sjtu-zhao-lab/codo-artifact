...
#include "codo/Dialect/CodoInterfaces.cpp.inc"
...
OpFoldResult codo::ToStreamOp::fold(FoldAdaptor adaptor) {
...
OpFoldResult codo::ToValueOp::fold(FoldAdaptor adaptor) {
...
LogicalResult ScheduleOp::verify() {
  ...
  if (getIsLegal())
    for (auto &op : getOps())
      ...
      if (!isa<NodeOp, BufferOp, ConstBufferOp>(op)) {
