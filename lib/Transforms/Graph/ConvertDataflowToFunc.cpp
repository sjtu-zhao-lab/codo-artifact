...
struct SplitScheduleExternalBufferAccess : public OpRewritePattern<ScheduleOp> {
  ...
  LogicalResult matchAndRewrite(ScheduleOp schedule,
    ...
    for (auto arg : args) {
      ...
      //   arg.dump();
      //   llvm::errs() << "OpOperand:\n";
      //   llvm::errs() << "  Owner: " << user.getOwner()->getName() << "\n";
      //   llvm::errs() << "  Operand number: " << user.getOperandNumber() << "\n";
      //   llvm::errs() << "  Value: " << user.get() << "\n";
      ...
      // int count = std::distance(uses.begin(), uses.end());
      // llvm::outs()<<count<<"\n";
...
struct ConvertNodeToFunc : public OpRewritePattern<NodeOp> {
  ...
  LogicalResult matchAndRewrite(NodeOp node,
    ...
    if (!node.hasHierarchy() &&
        llvm::hasSingleElement(node.getOps<affine::AffineForOp>()))
...
std::unique_ptr<Pass>
codo::createConvertDataflowToFuncPass(bool splitExternalAccess) {
