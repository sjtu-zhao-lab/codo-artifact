...
struct BackwardFuseOp : public OpRewritePattern<OpType> {
  ...
  LogicalResult matchAndRewrite(OpType op,
    ...
    for (Value operand : op->getOperands()) {
      if (auto task = operand.getDefiningOp<TaskOp>()) {
    ...
    TaskOp targetTask = taskDefOps.back();

    bool allOperandsDom = llvm::all_of(op->getOperands(), [&](Value v) {
      if (auto defOp = v.getDefiningOp())
        return DT.dominates(targetTask, defOp) || targetTask->isProperAncestor(defOp);
...
//     // Filter out invalid cases: ensure all operands are dominated by the chosen task.
//     llvm::sort(taskDefOps, [&](auto a, auto b) { return DT.dominates(a, b); });
//     TaskOp targetTask = taskDefOps.back();
...
struct ForwardFuseGenericOp : public OpRewritePattern<linalg::GenericOp> {
  ...
  LogicalResult matchAndRewrite(linalg::GenericOp op,
    ...
    if (op.getOutputs().size() == 1 &&
      ...
      if (output.getDefiningOp<arith::ConstantOp>())
        matched = true;
    }
...
struct CreateDataflowFromLinalg
  ...
  CreateDataflowFromLinalg(std::string hlsTopFunc) { topFunc = hlsTopFunc; }

  void getDependentDialects(mlir::DialectRegistry& registry) const override {
      registry.insert<mlir::codo::CodoDialect>();
  }
  void runOnOperation() override {
    ...
    // FIXME: We suppose that the target code of this pass only contains
    //        one (top) function.
    // llvm::outs()<<"the name of the top function is: "<<topFunc<<"\n";
    func.setSymName(topFunc);
    setTopFuncAttr(func);
    ...
    // patterns.add<OutlineRootInterface<linalg::FillOpInterface>>(context);
    populateForwardBackwardFusePatterns(patterns);
    (void)applyPatternsAndFoldGreedily(func, std::move(patterns));

    // This part aims to deal with the remaining unwrapped GenericOps.
    patterns.clear();
    patterns.add<OutlineRootOp<linalg::GenericOp>>(context);
    populateForwardBackwardFusePatterns(patterns);
    (void)applyPatternsAndFoldGreedily(func, std::move(patterns));

    // This part aims to deal with the remaining unwrapped MemrefOps and InsertSliceOps.
    patterns.clear();
    patterns.add<OutlineRootOp<tensor::InsertSliceOp>>(context);
    populateForwardBackwardFusePatterns(patterns);
    (void)applyPatternsAndFoldGreedily(func, std::move(patterns));
...
std::unique_ptr<Pass> codo::createCreateDataflowFromLinalgPass(std::string hlsTopFunc) {
  return std::make_unique<CreateDataflowFromLinalg>(hlsTopFunc);
}
