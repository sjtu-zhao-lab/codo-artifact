...
void codoEstimator::getPartitionIndices(Operation *op) {
  ...
  for (auto operand : accessMap.getOperands()) {
    if (operandIdx < accessMap.getNumDims()) {
      ...
      if (isAffineForInductionVar(operand))
        step = getForInductionVarOwner(operand).getStep().getSExtValue();
...
void codoEstimator::estimateLoadStoreTiming(Operation *op, int64_t begin) {
  auto access = MemRefAccess(op);
...
int64_t codoEstimator::getResMinII(int64_t begin, int64_t end,
                                       MemAccessesMap &map) {
  int64_t II = 1;
  for (auto &pair : map) {
...
int64_t codoEstimator::getDepMinII(int64_t II, func::FuncOp func,
                                       MemAccessesMap &map) {
  for (auto &pair : map) {
...
int64_t codoEstimator::getDepMinII(int64_t II, AffineForOp forOp,
                                       MemAccessesMap &map) {
  AffineLoopBand band;
          ...
          if (hasDependence(result)) {
            ...
            if (dstMuxSize > 3 || srcMuxSize > 3) {
              ...
              for (auto i = depComps.rbegin(); i < depComps.rend(); ++i) {
                ...
                if (ub >= 0)
                  distance +=
                      accumTrips.back() * max(lb, (int64_t)0) / loop.getStep().getSExtValue();
                else {
                  distance = -1;
                  break;
                }
...
bool codoEstimator::visitOp(AffineForOp op, int64_t begin) {
...
bool codoEstimator::visitOp(AffineIfOp op, int64_t begin) {
  auto end = begin;
...
bool codoEstimator::visitOp(scf::IfOp op, int64_t begin) {
  auto end = begin;
...
bool codoEstimator::visitOp(func::CallOp op, int64_t begin) {
  ...
  codoEstimator estimator(latencyMap, dspUsageMap, depAnalysis);
...
TimingAttr codoEstimator::estimateBlock(Block &block, int64_t begin) {
...
void codoEstimator::reverseTiming(Block &block) {
...
void codoEstimator::initEstimator(Block &block) {
...
ResourceAttr codoEstimator::calculateResource(Operation *funcOrLoop) {
...
void codoEstimator::estimateFunc(func::FuncOp func) {
...
void codoEstimator::estimateLoop(AffineForOp loop, func::FuncOp func) {
...
void codo::getLatencyMap(llvm::json::Object *config,
...
void codo::getDspUsageMap(llvm::json::Object *config,
...
namespace {
struct QoREstimation : public codo::QoREstimationBase<QoREstimation> {
  ...
  void runOnOperation() override {
    ...
    for (auto func : module.getOps<func::FuncOp>())
      if (hasTopFuncAttr(func))
        codoEstimator(latencyMap, dspUsageMap, true).estimateFunc(func);
  }
};
} // namespace

std::unique_ptr<Pass>
codo::createQoREstimationPass(std::string qorTargetSpec) {
