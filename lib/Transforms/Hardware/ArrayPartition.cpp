...
bool codo::applyArrayPartition(Value array, ArrayRef<unsigned> factors,
  ...
  if (!arrayType || isExtBuffer(array) || !arrayType.hasStaticShape() ||
    ...
    return false;
  // if(array.getStringAttr()==StringRef("opted")){
  //   return;
  // }
  // array.setStringAttr(StringRef("opted"));
...
/// targeted function.
bool codo::applyAutoArrayPartition(func::FuncOp func, DenseMap<Value, SmallVector<Partition, 4>> &partitionsMap, unsigned threshold) {
  // partitionsMap.clear();
  ...
  for (auto arg : func.getArguments()) {
    if (auto memrefType = arg.getType().dyn_cast<MemRefType>()) {
      auto &partitions = partitionsMap[arg];
      // Initialize partitions as none if empty.
      if (partitions.empty())
        partitions = SmallVector<Partition, 4>(
            memrefType.getRank(), Partition(PartitionKind::NONE, 1));
      // Traverse and analyze the argument like normal memrefs below.
      // Since function arguments are accessed throughout the function, 
      // we will later analyze their access pattern in all target blocks.
    }
  }
  ...
  for (auto block : targetBlocks) {
    ...
    for (auto [memref, loadStores] : accessesMap) {
      ...
      auto access_number = loadStores.size();
      ...
      for (int64_t dim = 0; dim < memrefType.getRank(); ++dim) {
        ...
        // if(kind == PartitionKind::CYCLIC &&  access_number>2 && access_number<32 && maxDistance == 1){
        //   llvm::outs()<<"enter test of dim: "<<dim<<"\n";
        //   int64_t dimSize = memrefType.getShape()[dim];
        //   int tempfactor = std::min((int64_t)access_number/2+1, dimSize);
        //   factor = std::max((int64_t)tempfactor, factor);
        //   // factor = memrefType.getShape()[dim];
        // }
        // llvm::outs() << "\nStretegy: "
  ...
  func.walk([&](func::CallOp op) {
    ...
    // llvm::outs()<<partitionsMap.size()<<"\n";
    // Apply array partition to the sub-function.
    applyAutoArrayPartition(subFunc, partitionsMap, threshold);
    // llvm::outs()<<partitionsMap.size()<<"\n";
...
struct ArrayPartition : public ArrayPartitionBase<ArrayPartition> {
  ...
  void runOnOperation() override {
    ...
    llvm::outs()<<topFunc.getName()<<"\n";
    DenseMap<Value, SmallVector<Partition, 4>> partitionsMap;
    applyAutoArrayPartition(topFunc, partitionsMap, threshold);
  }
};
} // namespace

std::unique_ptr<Pass> codo::createArrayPartitionPass(unsigned threshold) {
