...
struct LoopFusion : public AffineLoopFusionBase<LoopFusion> {
  ...
  LoopFusion(double computeToleranceThreshold, unsigned fastMemorySpace,
             ...
             enum codo::AffineFusionMode affineFusionMode) {
...
void LoopFusion::runOnBlock(Block *block) {
  llvm::outs() << "Running fusion on block\n";
...
void LoopFusion::runOnOperation() {
  ...
  // order).
  llvm::outs() << "Running fusion on runOnOperation\n";
...
// std::unique_ptr<Pass> codo::createAffineLoopFusionPass(
//     unsigned fastMemorySpace, uint64_t localBufSizeThreshold,
//     bool maximalFusion, enum affineFusionMode affineFusionMode) {
