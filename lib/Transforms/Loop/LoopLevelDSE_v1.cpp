...
namespace {
struct LoopLevelDSE_v1 : public LoopLevelDSE_v1Base<LoopLevelDSE_v1> {

  LoopLevelDSE_v1() = default;
  LoopLevelDSE_v1(int dseMaxLoopParallelismFactor, int balance_degree, int downscale_degree) { 
    MaxLoopParallelismFactor = dseMaxLoopParallelismFactor;
    balanceDegree = balance_degree;
    downscaleDegree = downscale_degree;
  }
        ...
        for (int i = 0; i < band.size();i++)
        {
          ...
          for (int i = 1; i < upperBound; ++i)
          {
            if (i != upperBound && upperBound % i == 0 && i <= MaxLoopParallelismFactor)
            {
              factor = i;
              if(balanceDegree!=0){
                if(upperBound>=768 && upperBound % (i*balanceDegree*4) == 0){
                  factor = factor*balanceDegree*4;
                }
                else if(upperBound>=512 && upperBound % (i*balanceDegree*2) == 0){
                  factor = factor*balanceDegree*2;
                }
              }
              // else if(upperBound>=256 && upperBound % (i*balanceDegree) == 0){
              //   factor = factor*balanceDegree;
              // }
              // llvm::outs()<<"factor: "<<i<<"\n";
            }
            
          }
          if(downscaleDegree == 1){
...
std::unique_ptr<Pass> codo::createLoopLevelDSE_v1Pass(int dseMaxLoopParallelismFactor, int balance_degree, int downscale_degree) {
  return std::make_unique<LoopLevelDSE_v1>(dseMaxLoopParallelismFactor, balance_degree, downscale_degree);
}